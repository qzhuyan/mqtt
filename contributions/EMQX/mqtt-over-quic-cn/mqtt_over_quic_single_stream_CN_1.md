...
| Inherited rule | Summary and source |
|:---------------|:-------------------|
| <a id="quic-alpn"></a>ALPN | ALPN offers and selection follow [RFC7301] §3, with negotiation failure handled using `no_application_protocol` (`0x0178`) as specified in [RFC9001] §8.1. |
| <a id="quic-early-data"></a>Early data | ALPN/PSK binding, early-data acceptance and rejection, and resetting stream-bound state after rejection follow [RFC8446] §4.2.10 and [RFC9001] §§4.6.2–4.6.3. |
| <a id="quic-stream-operations"></a>Stream operations | Stream operations, terminal-state restrictions, completion, and retransmission follow [RFC9000] §§2.4, 3, 4.4, and 13.3. |
| <a id="quic-stream-credit"></a>Stream credit | Stream-count credit is cumulative and cannot be revoked, as specified in [RFC9000] §§4.6 and 19.11. |
| <a id="quic-connection-termination"></a>Connection termination | QUIC connection termination, including handshake-dependent close-frame restrictions, follows [RFC9000] §10. |
| <a id="quic-keepalive"></a>Transport keepalive | QUIC idle-timeout calculation, keepalive operation, and PING frames follow [RFC9000] §§10.1 and 19.2. |
| <a id="quic-security"></a>Transport security | TLS protection and peer authentication follow [RFC9001] §§4.4 and 5. |
| <a id="quic-replay"></a>Replay exposure | 0-RTT data is vulnerable to replay, with transport-level protections and limitations defined in [RFC8446] §8 and [RFC9001] §9.2. |
...
| Inherited rule | Summary and source |
|:---------------|:-------------------|
| <a id="mqtt-establishment"></a>Connection establishment | CONNECT sequencing, CONNACK timeout, and CONNACK processing follow [MQTT5] §§3.1–3.2 or [MQTT311] §§3.1–3.2. |
| <a id="mqtt-packet-framing"></a>Packet framing | MQTT packet encoding and Remaining Length framing follow [MQTT5] §2 or [MQTT311] §2, independently of QUIC STREAM-frame boundaries ([RFC9000] §2.2). |
| <a id="mqtt-disconnect-5"></a>MQTT 5.0 DISCONNECT | DISCONNECT eligibility, encoding, and sender-role restrictions follow [MQTT5] §3.14, including the server's successful-CONNACK prerequisite. |
| <a id="mqtt-disconnect-311"></a>MQTT 3.1.1 DISCONNECT | DISCONNECT is a client-to-server packet without a reason code or properties, as defined in [MQTT311] §3.14. |
| <a id="mqtt-disconnect-effects"></a>DISCONNECT effects | Sending DISCONNECT ends further MQTT transmission and requires Network Connection closure, with receive-side and Will handling specified in [MQTT5] or [MQTT311] §§3.1.2.5 and 3.14.4. |
| <a id="mqtt-takeover"></a>Session takeover | Session takeover follows [MQTT5] §3.1.4 or [MQTT311] §3.1.4. |
| <a id="mqtt-session-5"></a>MQTT 5.0 Session State | Session retention and deletion follow Clean Start, Session Expiry Interval, and stored-state rules in [MQTT5] §§3.1.2.4, 3.1.2.11.2, and 4.1. |
| <a id="mqtt-session-311"></a>MQTT 3.1.1 Session State | Session retention and deletion follow Clean Session ([MQTT311] §3.1.2.4) and operational storage limits (§4.1), without a Session Expiry Interval property. |
| <a id="mqtt-keepalive"></a>MQTT keepalive | MQTT Keep Alive and PINGREQ/PINGRESP obligations follow [MQTT5] or [MQTT311] §§3.1.2.10 and 3.12–3.13, including the MQTT 5.0 Server Keep Alive override in [MQTT5] §3.2.2.3.14. |
| <a id="mqtt-protocol-errors"></a>Protocol errors | Protocol-error closure, MQTT notifications, and reason selection follow [MQTT5] §4.13.1 (including more specific rules such as §4.12.1) or [MQTT311] §4.8, subject to the DISCONNECT rules above. |
...
MQTT connection establishment follows the [inherited MQTT rules](#mqtt-establishment).
...
Apply the [inherited packet-framing rules](#mqtt-packet-framing) to this stream.
...
using the [inherited ALPN rules](#quic-alpn). This section specifies the
...
`mqtt-next` and `mqtt`), using the [ALPN offer rules](#quic-alpn).
...
follows the [ALPN failure rules](#quic-alpn).
...
for `mqtt`, subject to the [inherited early-data rules](#quic-early-data), the
...
[inherited early-data rules](#quic-early-data), the client MUST treat the early
...
[inherited connection-establishment rules](#mqtt-establishment).

### 6.1.1 CONNACK Processing

Apply the [inherited CONNACK processing rules](#mqtt-establishment); this
...
frames during idle periods, using the [transport keepalive rules](#quic-keepalive).
The [MQTT keepalive requirements](#mqtt-keepalive) also apply and MUST NOT be
...
[stream operations](#quic-stream-operations) and
[connection-termination rules](#quic-connection-termination).
...
1. The client MUST send `MQTT.DISCONNECT` over the stream, applying the
   [MQTT 5.0](#mqtt-disconnect-5) or [MQTT 3.1.1](#mqtt-disconnect-311)
...
1. After sending `MQTT.DISCONNECT` on the graceful path, the sender MUST
   ...
   applying the inherited [DISCONNECT effects](#mqtt-disconnect-effects).
...
3. Each endpoint MUST await normal completion of both stream directions using
   the [inherited stream-state rules](#quic-stream-operations), including
   ...
   [stream-state rules](#quic-stream-operations).
4. Each endpoint MUST enforce a finite, positive, configurable graceful
   ...
   under the [connection-termination rules](#quic-connection-termination),
...
`STOP_SENDING` follows the [stream-operation rules](#quic-stream-operations).
...
apply the inherited [Will handling](#mqtt-disconnect-effects) and
...
- **Local cancellation:** Use `RESET_STREAM` to abort the local sending
  ...
  under the [stream-operation rules](#quic-stream-operations), with reason
...
restrictions of the corresponding [MQTT 5.0 DISCONNECT reason](#mqtt-disconnect-5)
or [CONNACK reason](#mqtt-establishment).
...
Responses to `STOP_SENDING` follow the [stream-operation rules](#quic-stream-operations).
...
[MQTT notification constraints](#mqtt-disconnect-311). An unrecognized code
...
3. **ALPN negotiation fails:** After handling the
   [ALPN failure](#quic-alpn), the client MUST follow the
...
The broker MUST apply the [inherited session-takeover rules](#mqtt-takeover)
...
- **MQTT 5.0:** Use the takeover notification specified by the
  [inherited takeover rules](#mqtt-takeover) and complete shutdown under
...
not yield `mqtt`: either negotiation fails under the [ALPN rules](#quic-alpn),
...
the broker MUST apply the inherited [MQTT 5.0](#mqtt-session-5) or
[MQTT 3.1.1](#mqtt-session-311) Session State rules, as applicable.
...
client closure recommendation in the [inherited error rules](#mqtt-protocol-errors).
...
1. **Send direction:** The detecting endpoint MUST abort its sending direction
   ...
   [inherited stream-state rules](#quic-stream-operations); FIN is not a
...
2. **Receive direction:** The detecting endpoint MUST abort MQTT reads and send
   ...
   data nor been reset, under the [stream-operation rules](#quic-stream-operations).
...
[inherited error rules](#mqtt-protocol-errors).
...
Use the [inherited MQTT error notifications](#mqtt-protocol-errors), subject
to [MQTT 5.0 DISCONNECT eligibility](#mqtt-disconnect-5).
...
Apply the [MQTT 3.1.1 notification constraints](#mqtt-disconnect-311) without
...
[normal MQTT effects](#mqtt-disconnect-effects).
...
complete transport cleanup using the [inherited stream operations](#quic-stream-operations).
...
[stream-credit rules](#quic-stream-credit) to avoid this race:
...
the [connection-termination rules](#quic-connection-termination). Use an
...
Apply the inherited [transport security](#quic-security) and
[replay-protection rules](#quic-replay), together with the profile requirements below.
